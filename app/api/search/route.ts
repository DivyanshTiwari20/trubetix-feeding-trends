...
export async function GET(req: NextRequest) {
  ...
  const entity = searchParams.get('entity') ?? searchParams.get('q') ?? '';
  ...
  try {
    ...
    return NextResponse.json({ entity: r.entity, platform: r.platform, range: r.range, indexedResultCount: r.indexedResultCount, analyzedContentCount: r.analyzedContentCount, thirdPartyMentionCount: r.thirdPartyMentionCount, publishedCount: r.publishedCount, content: r.content, debug: r.debug }, { status: 200 });
  } catch (e:any) { return NextResponse.json({ error: e?.message ?? 'Search failed' }, { status: 500 }); }
}

export async function POST(req: NextRequest) {
  const j = await req.json().catch(()=>({}));
  const entity = j.entity ?? j.q ?? '';
  if (!entity) return NextResponse.json({ error: 'entity required' }, { status: 400 });
